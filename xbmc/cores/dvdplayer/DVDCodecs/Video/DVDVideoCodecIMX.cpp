...
{
  if (m_drainMode == drain || !m_vpuHandle)
    return;

  m_drainMode = drain;
  VpuDecInputType config = drain == -1 ? VPU_DEC_IN_DRAIN : drain;
  if (VPU_DEC_RET_SUCCESS != VPU_DecConfig(m_vpuHandle, VPU_DEC_CONF_INPUTTYPE, &config))
    CLog::Log(LOGERROR, "%s - iMX VPU set drain mode failed.\n", __FUNCTION__);
#if defined(IMX_PROFILE) || defined(TRACE_FRAMES)
...
{
  if (m_dropRequest == skip || !m_vpuHandle)
    return;

  m_dropRequest = skip;
  VpuDecSkipMode config = skip == -1 ? VPU_DEC_SKIPB : skip;
  if (VPU_DEC_RET_SUCCESS != VPU_DecConfig(m_vpuHandle, VPU_DEC_CONF_SKIPMODE, &config))
...

  {
    ...
    while (true) // Decode as long as the VPU consumes data
    {
      ...
      // VPU decoding init OK : We can retrieve stream info
      {
        ...
        {
          VpuBufferNode d;
          d.pVirAddr = d.pPhyAddr = 0;
          d.nSize = 0;

          SetDrainMode(VPU_DEC_IN_DRAIN);
          ret = VPU_DecDecodeBuf(m_vpuHandle, &d, &decRet);
          SetDrainMode(VPU_DEC_IN_NORMAL);
        }
      } //VPU_DEC_INIT_OK
...
void CDVDVideoCodecIMX::SetDropState(bool bDrop)
{
  ...
  if (m_dropRequest < 1 && m_vpuFrameBufferNum && m_speed != DVD_PLAYSPEED_PAUSE && m_frameInfo.pExtInfo)
...
{
  static std::forward_list<CDVDVideoCodecIMXBuffer*>::iterator it;
  ...
  if (count == 1)
  {
    ...
    it = m_recycleBuffers.insert_after(m_recycleBuffers.empty() ? m_recycleBuffers.before_begin() : it, this);
#ifdef TRACE_FRAMES
...
void CIMXContext::Blit(CIMXBuffer *source_p, CIMXBuffer *source, uint8_t fieldFmt, int page, CRect *dest)
{
  ...
  CLog::Log(LOGDEBUG | LOGVIDEO, "+B  %f\n", recalcPts(((CDVDVideoCodecIMXBuffer*)source)->GetPts()));
#endif
...
#ifdef IMX_PROFILE_BUFFERS
  unsigned long long after = XbmcThreads::SystemClockMillis();
  CLog::Log(LOGDEBUG | LOGVIDEO, "+P  %f  %d\n", recalcPts(((CDVDVideoCodecIMXBuffer*)ipu.current)->GetPts()), (int)(after-before));
#endif
